...
const IdentifiantDiv = styled.div`
    background-color: ${(props) =>
        backgroundIDMapping[props.types] ?? backgroundIDMapping.default};
`
    ...
    return (
        ...
        <div className="all">
            ...
            <ul className="imj-pokedex" >
                {listPoke?.map((pokemon) => (!activeTypes || activeTypes === pokemon.types[0].name || !activeTypes || activeTypes === pokemon.types[1]?.name) && (
                    <div>
                        <div className="bloc-pokedex">
                            <TypesDiv types={pokemon.types[0].name} key={pokemon.pokedex_id} className="imj-carte" onClick={() => togglePopup(pokemon.pokedex_id)} >

                                <IdentifiantDiv types={pokemon.types[1]?.name ?? pokemon.types[0].name} className="imj-number">N° {pokemon.pokedex_id}</IdentifiantDiv>
